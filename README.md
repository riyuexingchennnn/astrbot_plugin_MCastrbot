...
- **自主行为**：机器人可待机、跟随玩家，或独立地自动攻击附近的敌对生物并按优先顺序吃背包中的食物。
...
需要让机器人执行游戏操作时，把玩家名填入“管理员玩家名列表”，开启“允许 LLM 控制机器人行为”和“允许管理员让机器人调用 Agent 工具”；服务器也要授予机器人相应的游戏权限。机器人执行采集、合成等任务时会请求生存模式，任务结束后会保持当前游戏模式。`idle` 表示待机，`follow` 只跟随指定玩家，`auto` 只自动攻击附近敌对生物并自动进食，不跟随玩家。开启自动战斗工具会切入 `auto`；关闭时从 `auto` 回到 `idle`。睡觉工具会先切入 `idle`，完成后仍保持待机。
