...
让 Minecraft 玩家直接和 AstrBot 聊天：提到唤醒词的公屏消息得到公屏回复，`/tell` 消息得到私聊回复。插件还提供机器人状态面板，以及看向玩家、扫描周围和附近移动等操作。
...
- 玩家在 MC 公屏或 `/tell` 正文提到唤醒词（默认 `Fairy`，忽略大小写）才会得到回复；公屏回复走公屏，私聊回复走 `/msg`。唤醒词列表为空时回复所有消息；可单独关闭公屏回复。
- 在插件详情页打开 **dashboard**，查看连接状态、游戏状态、在线玩家、聊天和运行日志。
- 回复太长时，可在插件设置中启用**分段回复**，调整字数阈值、断句规则和发送间隔。
- 如果服务器的 `tell` Mod 使用特殊消息格式，可在设置中填写自定义匹配正则（先捕获玩家名，再捕获正文）。
